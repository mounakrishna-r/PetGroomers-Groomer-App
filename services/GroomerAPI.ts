...
class GroomerAPI {
  ...
  constructor() {
    ...
    this.api.interceptors.response.use(
      ...
      async (error) => {
        ...
        // Handle 401 (Unauthorized) or 404 (Groomer not found/deleted)
        if (status === 401 || (status === 404 && errorMessage.includes('Groomer account not found'))) {
          console.log('🔒 Groomer account invalid or deleted - clearing authentication data');
          ...
          // Reject with specific error for UI to handle
          const customError = new Error('GROOMER_DELETED');
  ...
  async validateGroomerExists(groomerId: number): Promise<ApiResponse<boolean>> {
    try {
      const response: AxiosResponse = await this.api.get(`/groomer/profile/${groomerId}`);
      return { success: true, data: true };
    } catch (error: any) {
      if (error.response?.status === 404 || error.message === 'GROOMER_DELETED') {
        return { success: false, error: 'Groomer account not found' };
      }
      // Network error or other issue - assume exists to avoid false logouts
      return { success: true, data: true };
    }
  }
